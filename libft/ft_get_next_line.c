...
/*   Updated: 2018/10/19 21:40:41 by tvarnier    ###    #+. /#+    ###.fr     */
/*                                                         /                  */
/*                                                        /                   */
/* ************************************************************************** */
...
static int		get_rest(int fd, t_rest **s)
{
	while (*s)
	{
		if ((*s)->fd == fd)
...
static int		ft_remove(t_rest **list, t_rest *target, t_rest *begin)
{
	t_rest		*tmp;
...
static int		ft_check_error(char **line, int fd, t_rest **s, t_rest **begin)
{
	*begin = *s;
	if (fd < 0 || !&(*line))
		return (1);
...
static int		get_next_line_next(t_rest **s, char **line, int fd,
		t_rest *begin)
{
	...
	if ((ret = read(fd, buf, BUFF_SIZE)) > 0)
	{
		...
		free(tmp);
		*s = begin;
		return (ft_get_next_line(fd, line, 1));
	}
	...
	if ((ft_remove(s, *s, begin)) == -1 && ret < 0)
		return (-1);
	return (0);
}

int				ft_get_next_line(const int fd, char **line, int check)
{
	static t_rest	*s;
	int				pos;
	char			*tmp;
